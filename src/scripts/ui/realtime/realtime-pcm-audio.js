...
export class RealtimePcmAudio {
  constructor({ onFrame, onError, onAudioLevel, microphoneAccess = microphonePermissionRecovery } = {}) {
    this.onFrame = onFrame; this.onError = onError; this.onAudioLevel = onAudioLevel; this.microphoneAccess = microphoneAccess; this.sources = new Set(); this.closed = true;
  }
  async open({ inputRate, outputRate, frameMs = 20, signal } = {}) {
    ...
    try {
      ...
      this.meter = createRealtimeAudioMeter({ context: this.context, onLevel: this.onAudioLevel });
      this.meter.attach('input', this.input); this.meter.attach('output', this.gain);
  ...
  setOutputMuted(value) { this.outputMuted = value; if (this.gain) this.gain.gain.value = value ? 0 : 1; this.meter?.setMuted('output', value); return true; }
  async close() {
    this.closed = true; this.clear(); this.stream?.getTracks().forEach(track => track.stop()); this.stream = null;
    await this.meter?.close(); this.meter = null;
