...
export const toolboxManagementMarkup = ({ items, catalog, ordering, shortcut, recording, shortcutError, expanded }) => {
  ...
  return `<div class="at-card-head">${button('back', '返回', { icon: 'back' })}<div class="at-card-heading"><strong>${e(t('快捷栏'))}</strong></div><button type="button" class="at-sort" data-key="sort" aria-pressed="${ordering}"${items.length < 2 && !ordering ? ' hidden' : ''}>${e(t(ordering ? '完成' : '排序'))}</button></div>
